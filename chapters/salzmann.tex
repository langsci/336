...
%\largerpage
The first argument for the DP-hypothesis comes from examples like the following, where in addition to the possessor there is also a prenominal determiner present (such structures are limited in English but frequent in other languages, e.g., Hungarian, cf., e.g., \citealt[270-276]{Abney:1987:Diss} and \citealt{Salzmann:2020:NP-DP}):
...
Even in languages like English where there there is no overt evidence for several specifiers of the same head being occupied at the same time, multiple specifier configurations can arise during the derivation, e.g., when a \textit{wh}-object undergoes successive-cyclic movement via Spec,vP.
...
\ea {
    \gll Jeder Schüler spielt Klavier.\\
        every student plays piano\\\hfill(German)
...
https://www.english-corpora.org/iweb/
 ...
 \gll ins Gras beissen\\
    in.the grass bite\\\hfill(German)
    \glt `die'
    }
\ex {
   ...
   \gll alle / beide / die Hände voll zu tun haben\\
        all  {} both {} the hands voll to do\\\hfill(German)
    \glt `Have one's hands full' \hfill
   }
   \z
    ...
    in a different grass bite like his father\\\hfill(German)
